function oneD_Wave(nCell,nSec,saveEvery,filename,tsInHours,doGlobalError)
...
%         [err errInd]=max(errVec(errInds)./PhiTest(errInds));
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%CONVERGE Ns%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        err=max(max(abs(nVec(1,:)-nVecOld(1,:))./nVec(1,:)));
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    PhiOld=PhiNew;
    if doGlobalError
        %we will need the K(y,g(y)) vector
        %we will also need the second time derivative (dKdt)
        %and derivatives wrt to the unknowns
        [K dKdy dKdphi]=computeK(nVec,PhiNew,SaxTest);
        [dphidn dMdN]=compute_dphidn(nVec);
        [dKdt sax_dot phi_dot]= compute_dKdt(dMdN,K,nVec,PhiNew,SaxTest);
        ge=ge+timeStep*((dKdy+dKdphi*dphidn)*(ge-K)+.5*dKdt);
%         figure(80)
%         hold on
%         plot(ge)
%         kill=12;
    ...
    if rem(outerCounter,saveEvery)==0
        ...
        zeroStore(saveCounter,:)=nVec(3,:);
        geStore(saveCounter,:)=ge;
        sax_dotStore(saveCounter-1)=sax_dot;
        phi_dotStore(saveCounter-1,:)=phi_dot;
        dNdtStore(saveCounter-1,:)=K;
        d2Ndt2Store(saveCounter-1,:)=dKdt;
    end
    if rem(outerCounter,writeEvery)==0
        ...
        if doGlobalError
            save geStore geStore;
            save sax_dotStore sax_dotStore;
            save phi_dotStore phi_dotStore;
            save dNdtStore dNdtStore;
            save d2Ndt2Store d2Ndt2Store;
        end
        cd ..
    end
end
