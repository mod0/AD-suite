function oneD_Wave_fromReference(nCell,nSec,saveEvery,filename)
...
ref0=zeroStore;
refPhi=phiStore;
clear nineStore eightStore zeroStore phiStore;
...
%%HACK INSERT HERE REFERENCE VALUES
for i=1:nCell
    PhiOld(i)=mean(refPhi(1,10*(i-1)+1:10*i));
    nVec(1,i)=mean(ref9(1,10*(i-1)+1:10*i));
    nVec(2,i)=mean(ref8(1,10*(i-1)+1:10*i));
    nVec(3,i)=mean(ref0(1,10*(i-1)+1:10*i));
end
...
while time<nSec
    ...
    PhiNew=PhiOld;
    for i=1:nCell
        PhiNew(i)=mean(refPhi(outerCounter,10*(i-1)+1:10*i));
        nVec0(1,i)=mean(ref9(outerCounter,10*(i-1)+1:10*i));
        nVec0(2,i)=mean(ref8(outerCounter,10*(i-1)+1:10*i));
        nVec0(3,i)=mean(ref0(outerCounter,10*(i-1)+1:10*i));
    end
    PhiOld=PhiNew;
    innerCounter=0;
    err=10;
