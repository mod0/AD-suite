function varargout = ode23t_hayes(ode,tspan,y0,options,varargin)
  ...
  while true                            % Evaluate the formula.
    ...
    if itfail
      ...
      if start
        ...
        errnew = 0.5*(znew-z);
        errOutVec=.5*abs((znew-z)./wt);
      else
        % The local truncation error is -(1/12)*y'''*h^3. 
        % Approximate by differentiating a cubic interpolant.
        % The interpolating polynomial is
            %P(s)=c1(s-t)(s-t_1)(s-t_2)+c2(s-tnew)(s-t_1)(s-t_2)+
                    % c3(s-tnew)(s-t)(s-t_2)+c4(s-tnew)(s-t)(s-t_1)
        %The third derivative of this function is the constant
            %P'''(s)=6(c1+c2+c3+c4)
      ...
      if err > rtol                     % Failed step
        ...
        needNewLU = true;
          ...
          errorOut=[errorOut;[tnew h errOutVec']];
        break;
      ...
      while next <= ntspan  
        if tdir * (tnew - tspan(next)) < 0
          if haveEventFcn && stop     % output tstop,ystop
            ...
            warning('Truncation Error for Terminating Event Not yet handled correctly');
          end
          break;
        end  
        nout_new = nout_new + 1;           
        tout_new = [tout_new, tspan(next)];
        if tspan(next) == tnew   %Landed right on it
          yout_new = [yout_new, ynew];
%           if start
%                 % The local truncation error is (1/2)*y''*h^2. 
%                 % Approximate by differencing the scaled derivative.
%                 errout_new = [errout_new, .5*(znew-z)];
%               else
%                 % The local truncation error is -(1/12)*y'''*h^3. 
%                 % Approximate by differentiating a cubic interpolant.
%                 % The interpolating polynomial is
...
%                 c4 = y_2/((t_2-t)*(t_2-t_1)*(t_2-tnew));
%                 est = -(c1 + c2 + c3 + c4)/2;
%                 errout_new = [errout_new, est*absh^3];
%             end
        else
          new_yout_new=ntrp23t(tspan(next),t,y,[],ynew,h,z,znew,...
                                        idxNonNegative);
          yout_new = [yout_new, new_yout_new];
%           if start
%                 % The local truncation error is (1/2)*y''*h^2. 
%                 % Approximate by differencing the scaled derivative.
%                 error('Truncation Error Interpolation during first time step not yet handled correctly');
%           else
%                 % The local truncation error is -(1/12)*y'''*h^3. 
%                 % Approximate by differentiating a cubic interpolant.
%                 % The interpolating polynomial is
...
%                 %The third derivative of this function is the constant
%                 %P'''(s)=6(c1+c2+c3+c4)
%                 c1 = new_yout_new/((tspan(next)-t_2)*(tspan(next)-t_1)*(tspan(next)-t));
%                 c2 = y/((t-tspan(next))*(t-t_2)*(t-t_1));
%                 c3 = y_1/((t_1-tspan(next))*(t_1-t)*(t_1-t_2));
%                 c4 = y_2/((t_2-t)*(t_2-t_1)*(t_2-tspan(next)));
%                 est = -(c1 + c2 + c3 + c4)/2;
%                 errout_new = [errout_new, est*absh^3];
% %                 %TESTING CODE
% %                 c1 = ynew/((tnew-t_2)*(tnew-t_1)*(tnew-t));
...
if output_sol
    solver_output{1}.truncErr=errout';
end
if nargout > 0
  varargout = solver_output;
end  
