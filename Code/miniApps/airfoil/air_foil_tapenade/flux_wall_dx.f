...
C   RW status of diff variables: res:in-out q:in x1:in x2:in
C
C
C
C
C
      SUBROUTINE FLUX_WALL_DX(x1, x1d, x2, x2d, q, qd, res, resd)
      IMPLICIT NONE
...
C
      ...
      REAL*8 x1d(2), x2d(2), qd(4), resd(4), dxd, dyd, rid, pd
C
C
      dxd = x1d(1) - x2d(1)
      dx = x1(1) - x2(1)
...
C
      resd(2) = resd(2) - pd*dy - p*dyd
      res(2) = res(2) - p*dy
      resd(3) = resd(3) + pd*dx + p*dxd
      res(3) = res(3) + p*dx
C
      RETURN
      END
