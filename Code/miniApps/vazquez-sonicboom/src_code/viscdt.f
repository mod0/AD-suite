...
c
               DO 1095 ivar=1,5
c
                  dx(ivar,is)      = (1.0 - pentel(is))*dxt(ivar) + 
     &            pentel(is)*0.5*(SIGN(1.0, dxt(ivar)) +
     &                            SIGN(1.0, dx(ivar,is)))*
     &            MIN(ABS(dxt(ivar)), ABS(dx(ivar,is)))
c
                  dy(ivar,is)      = (1.0 - pentel(is))*dyt(ivar) + 
     &            pentel(is)*0.5*(SIGN(1.0, dyt(ivar)) +
     &                            SIGN(1.0, dy(ivar,is)))*
     &            MIN(ABS(dyt(ivar)), ABS(dy(ivar,is)))
c
                  dz(ivar,is)      = (1.0 - pentel(is))*dzt(ivar) + 
     &            pentel(is)*0.5*(SIGN(1.0, dzt(ivar)) +
     &                            SIGN(1.0, dz(ivar,is)))*
