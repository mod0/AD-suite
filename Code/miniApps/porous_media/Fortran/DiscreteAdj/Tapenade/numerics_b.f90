...
!,disp_spmat
  ...
  INTERFACE ADD_X_B
      MODULE PROCEDURE ADDX_ELEM2_B
      MODULE PROCEDURE ADDX_DIAGONAL2_B
  END INTERFACE
  ...
  INTERFACE SPMAT_MULTIPLY_B
      MODULE PROCEDURE SPMAT_MULTIPLY_DIAGONAL2_B
      MODULE PROCEDURE SPMAT_MULTIPLY_VECTOR2_B
  END INTERFACE
...
CONTAINS
!  Differentiation of addx_elem2 in reverse (adjoint) mode (with options noISIZE):
...
!
  SUBROUTINE ADDX_ELEM2_B(innz, irow_index, irow_compressed, icol_index&
&   , ivalues, ivaluesb, x, row, col)
    IMPLICIT NONE
    DOUBLE PRECISION :: x
    INTEGER :: i, row, col
    INTEGER :: n, innz
...
!  Differentiation of addx_diagonal2 in reverse (adjoint) mode (with options noISIZE):
!   gradient     of useful results: ivalues
!   with respect to varying inputs: ivalues
!
...
! the column matrix
!
  SUBROUTINE ADDX_DIAGONAL2_B(innz, irow_index, irow_compressed, &
&   icol_index, ivalues, ivaluesb, x, diag)
    ...
    DOUBLE PRECISION, DIMENSION(7*n_) :: ivaluesb
    INTEGER, DIMENSION(n_ + 1) :: irow_compressed
  END SUBROUTINE ADDX_DIAGONAL2_B
!
! Subroutine adds x to a particular diagonal
! This subroutine is a bit flawed because at the time of construction of SPMAT
! , for other than the main diagonal, entries are skipped if they are 0.0 in
...
!  Differentiation of spmat_multiply_diagonal2 in reverse (adjoint) mode (with options noISIZE):
...
!
  SUBROUTINE SPMAT_MULTIPLY_DIAGONAL2_B(annz, arow_index, &
...
&         rvaluesb(i)
    ...
    ELSE
      dmatrixb = 0.D0
    END IF
  END SUBROUTINE SPMAT_MULTIPLY_DIAGONAL2_B
!
! This routine pre-multiplies a diagonal matrix by a sparse matrix
!
...
!  Differentiation of spmat_multiply_vector2 in reverse (adjoint) mode (with options noISIZE):
...
!
  SUBROUTINE SPMAT_MULTIPLY_VECTOR2_B(annz, arow_index, arow_compressed&
&   , acol_index, avalues, avaluesb, bvector, bvectorb, cvector, &
&   cvectorb, order)
    IMPLICIT NONE
    INTEGER :: i
    CHARACTER(len=3) :: order
...
&         cvectorb(acol_index(i))
    ...
    ELSE
      bvectorb = 0.D0
    END IF
  END SUBROUTINE SPMAT_MULTIPLY_VECTOR2_B
!
! The routine multiplies a vector by a sparse matrix (PRE/POST)
!
...
&   acol_index, avalues, avaluesb, b, bb, x, xb, solver_inner, &
...
!
  SUBROUTINE NEWTRAPH_B(s, sb, v, vb, q, qb, st, solver_inner, &
&   solver_outer, verbose)
    IMPLICIT NONE
    LOGICAL :: verbose
    INTEGER :: solver_inner, solver_outer
...
&                , dgvalues, dgvaluesb, g, gb, ds, dsb, solver_inner, &
&                solver_outer, verbose)
          bfwb = 0.D0
          ...
          fib = fib - gb
          fw = mw/(mw+mo)
          CALL SPMAT_MULTIPLY_VECTOR2_B(bnnz, brow_index, &
&                                 brow_compressed, bcol_index, bvalues, &
&                                 bvaluesb, fw, fwb, bfw, bfwb, 'PRE')
          ...
          CALL ADDX_DIAGONAL2_B(dgnnz, dgrow_index, dgrow_compressed, &
&                         dgcol_index, dgvalues, dgvaluesb, -1.0d0, 0)
          df = dmw/(mw+mo) - mw/(mw+mo)**2*(dmw+dmo)
          CALL POPINTEGER4(dgnnz)
          CALL POPREAL8ARRAY(dgvalues, 1400)
          CALL SPMAT_MULTIPLY_DIAGONAL2_B(bnnz, brow_index, &
&                                   brow_compressed, bcol_index, bvalues&
...
&                                   'PRE')
      ...
      CALL SPMAT_MULTIPLY_DIAGONAL2_B(annz, arow_index, arow_compressed&
&                               , acol_index, avalues, avaluesb, dtx, &
&                               dtxb, bnnz, brow_index, brow_compressed&
...
! Pressure Solver
!
  SUBROUTINE PRES_B(s, sb, q, qb, p, pb, v, vb, solver_inner, &
&   solver_outer, verbose)
    IMPLICIT NONE
!use print_active
    LOGICAL :: verbose
...
! point-wise multiply
    km = km*perm
    CALL TPFA_B(km, kmb, q, qb, p, pb, v, vb, solver_inner, solver_outer&
&         , verbose)
    kmb = perm*kmb
...
&   dmob)
    ...
    DOUBLE PRECISION, DIMENSION(n_) :: s_b
...
! rescale saturation
    ...
    IF (PRESENT(dmo) .AND. PRESENT(dmw)) THEN
      s_b = 0.D0
      s_b = 2*dmwb/(vw_*(1-sor_-swc_)) + 2*dmob/(vo_*(1-sor_-swc_))
      dmob = 0.D0
      dmwb = 0.D0
    ELSE
      s_b = 0.D0
    END IF
    s_b = s_b + 2*s_*mwb/vw_ - 2*(1-s_)*mob/vo_
    sb = sb + s_b/(1.0d0-sor_-swc_)
...
!
  SUBROUTINE RELPERM_SCALAR_B(s, sb, mw, mwb, mo, mob, dmw, dmo)
    ...
    DOUBLE PRECISION :: sb, mwb, mob, s_b
    DOUBLE PRECISION, OPTIONAL :: dmw, dmo
    INTRINSIC PRESENT
! rescale saturation
    s_ = (s-swc_)/(1.0d0-swc_-sor_)
    s_b = 2*s_*mwb/vw_ - 2*(1-s_)*mob/vo_
    sb = sb + s_b/(1.0d0-sor_-swc_)
...
!
  SUBROUTINE TPFA_B(k, kb, q, qb, p, pb, v, vb, solver_inner, &
&   solver_outer, verbose)
    IMPLICIT NONE
    LOGICAL :: verbose
    INTEGER :: solver_inner, solver_outer
...
! sparse matrix
    ...
    DOUBLE PRECISION :: tempb4(nx_-1, ny_, nz_)
    DOUBLE PRECISION :: tempb3(nx_, ny_-1, nz_)
    DOUBLE PRECISION :: tempb2(nx_, ny_, nz_-1)
    DOUBLE PRECISION :: tempb1(nx_-1, 10, 2)
    DOUBLE PRECISION :: tempb0(10, ny_-1, 2)
    DOUBLE PRECISION :: tempb(10, 10, nz_-1)
...
! ! Increment the 1,1 element of A
    arg1 = perm(1, 1, 1, 1) + perm(2, 1, 1, 1) + perm(3, 1, 1, 1)
...
! V.z
    tzb = 0.D0
    tempb = tz(:, :, 2:nz_)*vb(3, 1:nx_, 1:ny_, 2:nz_)
    pb(:, :, 1:nz_-1) = pb(:, :, 1:nz_-1) + tempb
    pb(:, :, 2:nz_) = pb(:, :, 2:nz_) - tempb
    tzb(:, :, 2:nz_) = tzb(:, :, 2:nz_) + (p(:, :, 1:nz_-1)-p(:, :, 2:&
&     nz_))*vb(3, 1:nx_, 1:ny_, 2:nz_)
    vb(3, 1:nx_, 1:ny_, 2:nz_) = 0.D0
    tyb = 0.D0
    tempb0 = ty(:, 2:ny_, :)*vb(2, 1:nx_, 2:ny_, 1:nz_)
    pb(:, 1:ny_-1, :) = pb(:, 1:ny_-1, :) + tempb0
    pb(:, 2:ny_, :) = pb(:, 2:ny_, :) - tempb0
    tyb(:, 2:ny_, :) = tyb(:, 2:ny_, :) + (p(:, 1:ny_-1, :)-p(:, 2:ny_, &
&     :))*vb(2, 1:nx_, 2:ny_, 1:nz_)
    vb(2, 1:nx_, 2:ny_, 1:nz_) = 0.D0
    txb = 0.D0
    tempb1 = tx(2:nx_, :, :)*vb(1, 2:nx_, 1:ny_, 1:nz_)
    pb(1:nx_-1, :, :) = pb(1:nx_-1, :, :) + tempb1
    pb(2:nx_, :, :) = pb(2:nx_, :, :) - tempb1
...
&          , avaluesb, q, qb, u, ub, solver_inner, solver_outer, verbose&
&         )
    CALL ADDX_ELEM2_B(annz, arow_index, arow_compressed, acol_index, &
&               avalues, avaluesb, arg1, 1, 1)
    CALL SPDIAGS_FVM_CSR_B(diags, diagsb, annz, arow_index, &
&                    arow_compressed, acol_index, avalues, avaluesb)
    diagsb(:, 1) = diagsb(:, 1) - diagsb(:, 4)
    diagsb(:, 2) = diagsb(:, 2) - diagsb(:, 4)
    ...
    tempb2 = -(tz_*tzb(1:nx_, 1:ny_, 2:nz_)/temp1**2)
    lb(3, :, :, 1:nz_-1) = lb(3, :, :, 1:nz_-1) + tempb2
    lb(3, :, :, 2:nz_) = lb(3, :, :, 2:nz_) + tempb2
    temp0 = l(2, 1:nx_, 1:ny_-1, 1:nz_) + l(2, 1:nx_, 2:ny_, 1:nz_)
    tempb3 = -(ty_*tyb(1:nx_, 2:ny_, 1:nz_)/temp0**2)
    lb(2, :, 1:ny_-1, :) = lb(2, :, 1:ny_-1, :) + tempb3
    lb(2, :, 2:ny_, :) = lb(2, :, 2:ny_, :) + tempb3
    temp = l(1, 1:nx_-1, 1:ny_, 1:nz_) + l(1, 2:nx_, 1:ny_, 1:nz_)
    tempb4 = -(tx_*txb(2:nx_, 1:ny_, 1:nz_)/temp**2)
    lb(1, 1:nx_-1, :, :) = lb(1, 1:nx_-1, :, :) + tempb4
    lb(1, 2:nx_, :, :) = lb(1, 2:nx_, :, :) + tempb4
    kb = 0.D0
    kb = -(lb/k**2)
...
!   RW status of diff variables: sigma:out oil:in-zero pc:(loc)
!                mu:out
  SUBROUTINE WRAPPER_B(ir, mu, mub, sigma, sigmab, q, s, p, v, st, pt, &
&   tt, nd, mw, mo, mt, pc, pcb, oil, oilb, solver_inner, solver_outer, &
&   verbose)
    USE GRID_B
    USE FLUID_B
    IMPLICIT NONE
...
!$openad independent(ir)
    ...
    CALL SIMULATE_RESERVOIR_B(q, qb, s, p, v, st, pt, tt, nd, mw, mo, mt&
&                       , pc, pcb, oil, oilb, solver_inner, solver_outer&
&                       , verbose)
    CALL INIT_FLW_TRNC_NORM_XIN_PT_OUT_B(ir, mu, mub, sigma, sigmab, q, &
&                                  qb)
    oilb = 0.D0
...
!
  SUBROUTINE INIT_FLW_TRNC_NORM_XIN_PT_OUT_B(ir, mu, mub, sigma, sigmab&
&   , q, qb)
    IMPLICIT NONE
!---------------------------------------------------------------------------
...
! model.
!
  SUBROUTINE SIMULATE_RESERVOIR_B(q, qb, s, p, v, st, pt, tt, nd, mw, mo&
&   , mt, pc, pcb, oil, oilb, solver_inner, solver_outer, verbose)
    USE GRID_B
    USE FLUID_B
    IMPLICIT NONE
    LOGICAL :: verbose
...
&                   , tempoil2b)
        ...
        CALL POPREAL8ARRAY(s, 200)
        CALL NEWTRAPH_B(s, sb, v, vb, q, qb, st, solver_inner, &
&                 solver_outer, verbose)
        CALL POPINTEGER4(k)
        tempoil2b = 0.D0
      END DO
      ...
      CALL PRES_B(s, sb, q, qb, p, pb, v, vb, solver_inner, solver_outer&
&           , verbose)
    END DO
