...
subroutine sparse_solve(A, b, x)
    ...
    call sparse_dummy_method(A, b, x)
...
subroutine sparse_jacobi_method(A, b, x)
    ...
    double precision, dimension(A%rows) :: b, x, x_old, main_diag, x_diff
...
!$omp end parallel
        ...
        ! get the difference before using it in a subroutine
        x_diff = x - x_old

        ! get the norm
        call dnrm2(x_diff, A%rows, nrm)
...
!
subroutine sparse_dummy_method(A, b, x)
        integer :: i
