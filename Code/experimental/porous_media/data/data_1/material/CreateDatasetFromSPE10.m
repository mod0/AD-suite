function CreateDatasetFromSPE10(filename, NX, NY, NZ)
    ...
    por = zeros(NX, NY, NZ);
    perm = zeros(3, NX, NY, NZ);
    sat = zeros(NX, NY, NZ);
    
    for i = 1:NX
        for j = 1:NY
            for k = 1:NZ
                por_temp = data.pU((i-1) * rnx + 1:i*rnx, ...
                                  (j-1) * rny + 1:j*rny, ...
                                  (k-1) * rnz + 1:k*rnz);
                por(i, j, k) = mean(por_temp(:));
                
                perm_temp = data.KU(1, (i-1) * rnx + 1:i*rnx, ...
                                  (j-1) * rny + 1:j*rny, ...
                                  (k-1) * rnz + 1:k*rnz);
                perm(1,i,j,k) = mean(perm_temp(:));
                
                perm_temp = data.KU(2, (i-1) * rnx + 1:i*rnx, ...
                                  (j-1) * rny + 1:j*rny, ...
                                  (k-1) * rnz + 1:k*rnz);
                perm(2,i,j,k) = mean(perm_temp(:));
                
                perm_temp = data.KU(3, (i-1) * rnx + 1:i*rnx, ...
                                  (j-1) * rny + 1:j*rny, ...
                                  (k-1) * rnz + 1:k*rnz);
                perm(3,i,j,k) = mean(perm_temp(:));
            end
        end
    end
    ...
    dimid_vol = netcdf.defDim(ncid,'vol',1);
    ...
    varid_vol = netcdf.defVar(ncid,'vol','double',dimid_vol);
    ...
    netcdf.putVar(ncid,varid_vol,vol);
