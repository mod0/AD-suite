...
REAL(w2f__8) BETA_FRIC_DUMMY(1 : 79)
type(active) :: F(1:79)
type(active) :: FEND
type(active) :: H(1:79)
...
! B(jj)%v=B_DUMMY(jj)%v
! H(jj)%v=H_DUMMY(jj)%v
! print *,b(jj)%v,b(jj)%d,h(jj)%v,h(jj)%d,u(jj)%v,u(jj)%d
 endif
! U(jj)%d=U_dummy(jj)%d
! U_DUMMY(jj)%v=U(jj)%v
enddo
...
!  CALL phi(U,B_DUMMY,H_DUMMY,BETA_FRIC_DUMMY)
  CALL phi(U,B,H,BETA_FRIC)
print *,'iteration', INT(OpenAD_Symbol_15)
do jj=1,79
 print *,b(jj)%v,b(jj)%d,h(jj)%v,h(jj)%d,u(jj)%v,u(jj)%d
enddo
...
do while (INT(OpenAD_Symbol_66).LE.INT(OpenAD_Symbol_65))
  ...
  UTMP(OpenAD_Symbol_122)%d = UTMP(OpenAD_Symbol_122)%d+U(OpenAD_Symbol_123)%d
  U(OpenAD_Symbol_123)%d = 0.0d0
